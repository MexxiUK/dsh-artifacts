...
- Copy rendered text: the copy button offers "Copy source" and "Copy rendered"
  for Markdown and HTML.
- Download all versions: the download button offers "Download current" and
  "Download all versions".
- Keyboard shortcuts: Esc closes the canvas, Cmd/Ctrl+Enter sends a question,
  Cmd/Ctrl+F finds.

### Changed

- HTML and options artifacts now run under a restrictive Content-Security-Policy
  that blocks network access (remote scripts, styles, images, and fetch/XHR).
- Downloads now include the version number in the filename.

### Fixed

- The version counter is now persisted to disk, so updating an artifact after a
  DeepSeek Harness restart works instead of failing with "unknown artifact_id".
